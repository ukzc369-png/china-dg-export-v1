...
export default function ProductDetailPage({ product, products, lang, onBack, onOpenProduct, onQuote }: { product: ProductSource; products: ProductSource[]; lang: Lang; onBack: () => void; onOpenProduct: (product: ProductSource) => void; onQuote: () => void }) {
  ...
  return <main className="pd-page">
    ...
    {detail.slug === "aniline" && <section className="pd-section pd-buyer-checklist"><div className="container pd-panel"><div><p className="pd-kicker">{lang === "en" ? "Inquiry Preparation" : "询价准备"}</p><h2>{lang === "en" ? "Aniline buyer checklist" : "苯胺买家资料清单"}</h2><p>{lang === "en" ? "Send these details so we can review the suitable supply, documents, packing and shipment route before quotation." : "请提供以下资料，以便报价前审核适用货源、文件、包装和出运路线。"}</p></div><ul>{(lang === "en" ? ["Required specification", "Order quantity", "Packing preference", "Destination port", "Industrial end use", "Required documents and timeline"] : ["所需指标", "采购数量", "包装偏好", "目的港", "工业最终用途", "文件要求和交期"]).map((item) => <li key={item}>{item}</li>)}</ul></div></section>}
